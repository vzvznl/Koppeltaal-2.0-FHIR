...
### Toegevoegd
- **KT2DeletePendingTask profiel**: nieuw Task-profiel voor de aankondigings-Task van het opschoningsproces (één per Patient × deelnemende applicatie). Vaste `code` (`delete-pending`, `1..1`, required gebonden aan `koppeltaal-task-code`), `intent = order`, verplichte `authoredOn` als aankondigingsmoment waarop de grace period start, `for` (KT2_Patient), `owner` en `requester` (KT2_Device), verplichte server-beheerde `restriction.period.end` (grace-deadline) en precies één coded `statusReason` voor de noodrem-reden. Drie invarianten dwingen af wat het profiel beschrijft: `kt2-dpt-status-reason` stelt de reden verplicht bij `status = on-hold` en verbiedt hem bij elke andere status, `kt2-dpt-delete-flow-marker` eist de server-owned `kt2-delete-flow`-marker in `meta.security` (de toegangsgrant tot de flow) en `kt2-dpt-grace-deadline` houdt de grace-deadline ná het aankondigingsmoment. Het profiel zet dezelfde elementen dicht als `KT2_Task` — het profiel erft van `Task` en moet dat dus zelf doen — met uitzondering van `statusReason` en `restriction`, die de aankondigings-Task juist nodig heeft. Daarbovenop staat `description` op `0..0`: applicaties zonder Task-leesrecht ontvangen deze Tasks via de `kt2-delete-flow`-marker, dus de coded `statusReason` hoort de enige vastgelegde reden te zijn. Reageren gebeurt via de native Task-lifecycle (`on-hold` als noodrem, `accepted` als groen licht); de server valideert de overgangen. Vervangt PR #67.
...
- **KoppeltaalDeleteHoldReason codesysteem + ValueSet**: gesloten redenlijst voor de noodrem (`Task.statusReason` op de KT2DeletePendingTask), met required binding en `text 0..0` — de reden is daarmee geen vrijetekstveld, wat van belang is omdat de Task domein-breed leesbaar is.
- **KoppeltaalDeletePendingTaskStatus ValueSet**: nieuwe ValueSet die `Task.status` op de KT2DeletePendingTask required bindt aan de vijf statussen die in de opschoon-flow voorkomen (`requested`, `on-hold`, `accepted`, `cancelled`, `completed`) in plaats van alle twaalf R4 task-statussen.
- **Voorbeelden**: `task-delete-pending` (aankondigings-Task), `task-delete-pending-on-hold` (noodrem met coded reden), `device-koppeltaalvoorziening` (Device van de Koppeltaalvoorziening) en de zes geaggregeerde opschoning-lifecycle AuditEvents `auditevent-opschoning-archive` / `-hold` / `-unhold` / `-grace-reset` / `-reactivate` / `-destroy` (ISO 21089 lifecycle-codes op `type`, `entity.what` altijd de Patient, de Koppeltaalvoorziening als `agent`); alle voorbeelden dragen de `kt2-delete-flow`-marker.
