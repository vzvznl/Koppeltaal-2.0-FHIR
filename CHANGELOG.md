...
Alles hieronder is ná de release van 0.16.4 (28 juli 2026) op main gekomen. Het stond eerder abusievelijk onder 0.16.4, terwijl die versie toen al was uitgebracht.
...
- **Voorbeeld**: `task-delete-pending-on-hold` toegevoegd — de noodrem met een coded reden, zodat die tak van de flow ook een voorbeeld heeft.

### Hersteld
- **KT2DeletePendingTask profiel**: gecorrigeerd na review. `code` is nu `1..1` en required gebonden aan `koppeltaal-task-code`; de overgeërfde FHIR-core voorbeeldbinding bevatte de vaste code `delete-pending` niet, waardoor de profielpagina naar de verkeerde codelijst wees. `status` is required gebonden aan de nieuwe KoppeltaalDeletePendingTaskStatus ValueSet. `authoredOn` is `1..1` als aankondigingsmoment waarop de grace period start. `statusReason.coding` is `1..1` en de verplichte `coding.display` is losgelaten naar `0..1`. Drie invarianten dwingen nu af wat het profiel al beschreef: `kt2-dpt-status-reason` (reden verplicht bij `status = on-hold`, verboden bij elke andere status), `kt2-dpt-delete-flow-marker` (de server-owned `kt2-delete-flow`-marker in `meta.security`) en `kt2-dpt-grace-deadline` (deadline ná `authoredOn`). Het profiel zet dezelfde elementen dicht als `KT2_Task`, plus `description`. `Title` verwijderd zodat de kop gelijk is aan de overige profielen.
- **KoppeltaalDeleteHoldReason ValueSet**: `^version` toegevoegd. De ValueSet declareert het `shareablevalueset`-profiel, dat `version` verplicht stelt (`1..1`), maar leverde die niet.
