...
The ActivityDefinition resource describes an eHealth activity that is available for assignment to a patient. When an eHealth activity is assigned to a patient, a Task resource is created. Sub-activities are included as contained resources that refer to the main task via the Task.partOf element.
