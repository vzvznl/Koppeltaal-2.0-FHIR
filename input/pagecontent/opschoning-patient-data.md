...
| 0.2.9 | 2026-09-25 | Afbakening bij patiëntverwijdering: behoud van Tasks met gewiste Patient-owner, optionele `issue-type#deleted`-tag zonder wijziging van `meta.lastUpdated`, verwijdering van CareTeam-participant-entries en blokkade bij tegenstrijdige patiënttoewijzingen. Aanvulling op het ontwerp ter besluitvorming. |
...
- **Scope**: patiëntgebonden gegevens uit het Patient Compartment, met de afbakening voor behouden resources hieronder, **met `AuditEvent` uitgesloten** — die overleeft als centraal record en mag de verwijderde `Patient/{id}` blijven refereren (referentiële integriteit op dat punt uitgezonderd). De Tasks van deze Patient (`Task` valt niet in het compartiment) worden **apart** mee-verwijderd: de `delete-pending`-Tasks én de historische `cancelled`-Tasks (die verwijzen nu naar een gewiste Patient). Tijdens een lopende cyclus blijft een `cancelled`-Task juist behouden (onderscheidt reactivering van een uitgevoerde verwijdering). Omvat o.a. Patient, RelatedPerson, CareTeam.
...
#### Verwijzingen vanuit behouden resources

**Patiëntgebonden gegevens en uitvoeringsverantwoordelijkheid.** Bij verwijdering van `Patient/A` bepaalt `Task.for`, `CareTeam.subject` respectievelijk `RelatedPerson.patient` welke patiëntgebonden resources worden gewist. Een `Patient` in `Task.owner` is de uitvoerder en hoeft niet dezelfde patiënt te zijn als `Task.for`. Alleen die owner-verwijzing is daarom geen grond om een Task van een andere patiënt mee te wissen. Dit onderscheid volgt uit de definities van [Task.for en Task.owner in FHIR R4](https://hl7.org/fhir/R4/task-definitions.html) en het [KT2Task-profiel](StructureDefinition-KT2Task.html).

Tasks met een te wissen patiëntgebonden CareTeam of RelatedPerson als owner worden in de wisselectie betrokken. Wijst hun `Task.for` naar een andere patiënt, dan **MOET** de voorziening de verwijdering van deze `Task` blokkeren totdat de toewijzing is opgehelderd.

**Een behouden Task.** Incidenteel kan een Task voor patiënt B blijven verwijzen naar een gewiste patiënt A als `owner`. De Task blijft dan ongewijzigd behouden, zodat `T_legacy` geen nieuwe activiteit voor B ziet. [FHIR R4](https://hl7.org/fhir/R4/http.html#delete) staat toe dat bij verwijdering verwijzingen achterblijven. Voor vindbaarheid kan [`meta.tag`](https://hl7.org/fhir/R4/resource.html#Meta) de bestaande code [`http://hl7.org/fhir/issue-type#deleted`](https://hl7.org/fhir/R4/codesystem-issue-type.html#issue-type-deleted) bevatten: een verwijzing naar verwijderde inhoud. Dit hergebruik van een OperationOutcome-code als tag is een Koppeltaal-afspraak en vereist geen profieluitbreiding. Voorwaarde is dat het toevoegen van de tag `meta.lastUpdated` niet wijzigt. Eventuele gekopieerde persoonsgegevens vragen afzonderlijke afhandeling.

**Een behouden CareTeam.** Van een CareTeam dat niet wordt gewist, **MOET** de voorziening de volledige participant-entry verwijderen wanneer `participant.member` naar een te wissen resource verwijst. Dit omvat ook bestaande Patient-memberships uit legacy-data; het verruimt de toegestane participants in het KT2CareTeam-profiel niet. `participant` heeft minimum nul: als de laatste participant verdwijnt, blijven CareTeam, subject en status behouden. Er wordt geen anonieme participant ingevoegd. Zie [FHIR R4 CareTeam.participant](https://hl7.org/fhir/R4/careteam-definitions.html#CareTeam.participant) en [KT2CareTeam](StructureDefinition-KT2CareTeam.html).
