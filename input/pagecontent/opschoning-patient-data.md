...
4. **Geaggregeerde AuditEvents — besloten. De delete-AuditEvents leggen de geaggregeerde proces-status vast (door de Koppeltaalvoorziening), niet elke losse Task.status-write per applicatie. hold volgt op de eerste handrem, unhold pas wanneer de laatste eraf is én het proces dan nog doorloopt (niet alle Tasks accepted); een tweede hold of gedeeltelijke release levert geen event op, en een accepted zonder voorafgaande hold evenmin.** Voltooit die laatste release het proces (alle `accepted`), dan volgt direct `destroy` i.p.v. `unhold`. **Grondslag: KT2-juridisch hoeft de verwijdering niet per client te worden onderbouwd. (Deze keuze was eerder gemaakt maar nooit in de IG beland.)**
...
6. **Discriminator op AuditEvent.type met standaard ISO 21089-codes — besloten.** De workflow-events worden gediscrimineerd via **standaard ISO 21089 record-lifecycle-codes op `AuditEvent.type`** (`archive`/`hold`/`unhold`/`reactivate`/`destroy`); `type` **is** een R4-zoekparameter (`AuditEvent?type=…`), dus zoeken en subscriben werkt zonder eigen CodeSystem. De flow als geheel blijft vindbaar via het `kt2-delete-flow`-`_security`-label. Een custom `kt2-audit-event`-CodeSystem en het gebruik van `entity.lifecycle` (géén zoekparameter) zijn hiermee vervallen; de eerder afgesproken ISO 21089-mapping (memo §3.8) is weer leidend.
