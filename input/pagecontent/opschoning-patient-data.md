...
4. **Geaggregeerde AuditEvents — besloten.** De delete-AuditEvents leggen de **geaggregeerde** proces-status vast (door de Koppeltaalvoorziening), niet elke losse `Task.status`-write per applicatie. `hold` volgt op de **eerste** handrem, `unhold` pas wanneer de **laatste** eraf is; een tweede hold of gedeeltelijke release levert geen event op, en een `accepted` zonder voorafgaande hold evenmin. Grondslag: KT2-juridisch hoeft de verwijdering **niet per client** te worden onderbouwd. (Deze keuze was eerder gemaakt maar nooit in de IG beland.)
