...
1. **Subtype `DCM#110126` (W4)** — afgedaan in TOP-KT-028 (v0.8): het subtype wordt gehandhaafd met een eigen display (het FHIR-label "Node Authentication" wijkt af van het gebruik als user-login); er is geen harde SHALL.
2. **Marker-code in het subscription-voorbeeld (W5)** — de canonieke CodeSystem-URI is inmiddels in de IG vastgelegd (`http://vzvz.nl/fhir/CodeSystem/koppeltaal-security-label`, code `kt2-delete-flow`); het `_security`-deel van de voorbeeld-criteria in W5 gebruikt nog `https://koppeltaal.nl/fhir/CodeSystem/security-label` en moet daarop worden aangepast (het `type`-deel staat vast).
...
5. **Mitigatie identificerende search-queries in het audit-log** — overgenomen uit TOP-KT-028 (v0.8; voorheen discussiepunt 9 aldaar): access-log-AuditEvents blijven ná de erase persoonsgegevens (bewaring o.g.v. de wettelijke logging-plicht, NEN 7513 / AVG art. 17 lid 3 sub b). Verdergaande mitigatie ligt aan de Topic 11-kant: identificerende parameterwaarden (zoals `identifier=…`) hashen of redigeren in `entity.query` — en in de clear-text-vastlegging van de query in `entity.description` in de referentie-implementatie. Spanningsveld: de immutability en NEN 7513-integriteit van het log en de bruikbaarheid voor incident-onderzoek; vergt een wijziging van Topic 11 én de implementatie. Te beleggen bij privacy/architectuur.
