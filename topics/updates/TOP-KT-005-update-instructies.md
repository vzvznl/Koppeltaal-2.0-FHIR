...
- **Motivatie**: dit is de kern van het voormalige TOP-KT-028-discussiepunt "marker formeel vastleggen in de autorisatiepagina's" (inmiddels afgevoerd — de vastlegging is als actiepunt bij deze instructies belegd). TOP-KT-005a beschrijft het autorisatiemodel (RBAC, `resource-origin`, search-/subscription-narrowing); de uitzondering hoort op dezelfde plek, in dezelfde begrippen. De tekst volgt de normatieve passages van TOP-KT-028 (Oplossingsrichting, "Toegang buiten de matrix" en "Status-lifecycle & server-validatie") en eisen 5 t/m 9 en 15 daaruit.
...
2. **Marker-code en borging**: de canonieke code en het CodeSystem zijn inmiddels in de IG vastgelegd (`http://vzvz.nl/fhir/CodeSystem/koppeltaal-security-label#kt2-delete-flow`); het werkvoorstel `https://koppeltaal.nl/fhir/CodeSystem/security-label` in W1–W3 en de voorbeeld-criteria moet daarop worden aangepast. Het voormalige TOP-KT-028-discussiepunt "marker formeel vastleggen" is afgevoerd — de vastlegging is als actiepunt bij déze instructies belegd; de bevestiging dat AuditEvent-read-only KT2-beleid is en de borging-details (server-owned marker; DPA-domein uit de Device-registratie; onafhankelijke autorisatie van doorsijpel-paden) lopen mee in de algemene besluitvorming over TOP-KT-028. W1, W2, W3 en E-nieuw-3/4/5 hangen hiervan af.
3. **Subtype `DCM#110126`** — afgedaan in TOP-KT-028 (v0.8): het subtype wordt gehandhaafd met een eigen display (het FHIR-label "Node Authentication" wijkt af); er is geen harde SHALL. E-nieuw-2 kan dienovereenkomstig worden verwerkt.
4. **Afhankelijk van besluit TOP-KT-028 discussiepunt 3**: wie "deelnemende applicaties" zijn (automatisch elke app in het domein, of expliciete opt-in) bepaalt de reikwijdte van de domein-brede lees-grant in W2 en E-nieuw-4. *Voorlopig besloten (19 aug 2026): automatisch — elke app in het domein.*
5. **Afhankelijk van besluit TOP-KT-028 discussiepunt 1**: de domein-brede leesbaarheid kan in een v2 worden versmald (footprint-based, AVG art. 19); de formulering "domein-breed" in W2/W3 zou dan wijzigen. *Voorlopig besloten (19 aug 2026): domein-breed bevestigd; versmalling blijft een v2-optie.*
